...
if inputFile.find('_filtro') != -1:
    baseFileName = inputFile.replace('.json', '').replace('_filtro', '')
else:
...
# Lee el archivo y lo asigna a una variable
contenedorJsonTuits = json.load(archivoAbierto)
...
if baseFileName.find('filtro') != -1:
    ...
    usuariosFileRoute = baseFileName
...
print '\nPublicaciones totales: ' + str(len(contenedorJsonTuits))
